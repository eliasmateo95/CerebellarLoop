function ComplexPlots(Type,Simulation,Data,Params)
if Type == "Coupled"
...
if Plots.CSTrigNoise == 'y'
    plotCStrigNoise(Name,SimData.IO_spikes,SimData.PC_noise,SimData.IO_Vs,SimData.Noise_t,Params.timepreceding,Params.save,SimParams.fname,Params.show,Params.dtime);
end
f1 = waitbar(1/9,char('CS Triggered Noise Plots Finished '+string(Simulation)));close(f)
% CS Triggered SS
if Plots.CSTrigSS == 'y'
    plotCStrigSS(Name,SimData.IO_spikes,SimData.PC_v,SimData.IO_Vs,SimData.Noise_t,SimData.PC_spikes,SimData.PC_firingrate,Params.timepreceding,Params.save,SimParams.fname,Params.show,Params.dtime);
end
f2 = waitbar(2/9,char('CS Triggered SS Plots Finished '+string(Simulation)));close(f1)
% CS Triggered DCN
if Plots.CSTrigDCN == 'y'
    plotCStrigDCN(Name,SimData.IO_spikes,SimData.IO_Vs,SimData.Noise_t,SimData.DCN_spikes,SimData.DCN_v,Params.timepreceding,Params.save,SimParams.fname,Params.show,Params.dtime);
end
f3 = waitbar(3/9,char('CS Triggered DCN Plots Finished '+string(Simulation)));
close(f2)
% DCN Spikes Triggered Noise 
if Plots.DCNTrigNoise == 'y'
    plotDCNtrigNoise(Name,SimData.DCN_spikes,SimData.PC_noise,SimData.DCN_v,SimData.Noise_t,Params.timepreceding,Params.save,SimParams.fname,Params.show,Params.dtime);
end
f4 = waitbar(4/9,char('DCN Triggered Noise Plots Finished '+string(Simulation)));close(f3)
% Firing Rates
if Plots.firing == 'y'
    firingplots(Name,SimData.PC_firingrate,SimData.DCN_firingrate,SimData.IO_firingrate,SimData.Noise_t,Params.window,Params.save,SimParams.fname,Params.show,Params.dtime);
end
...
if Plots.corrfiring == 'y'
    corrfiringplots(Name,SimData.PC_firingrate,SimData.DCN_firingrate,SimData.IO_firingrate,Params.save,SimParams.fname,Params.show,Params.dtime);
end
f6 = waitbar(6/9,char('Firing Frequency Correlation Plots Finished '+string(Simulation)));close(f5)
% SS CS Raster Plot
if Plots.raster == 'y'
    rasterplotPCIO(Name,SimData.IO_spikes,SimData.PC_spikes,SimData.IO_Vs,Params.timepreceding,Params.save,SimParams.fname,Params.show,Params.dtime);
end
...
if Plots.waterfallIOVs == 'y'
    waterfallIOVs(Name,SimData.IO_spikes,SimData.IO_Vs,Params.timepreceding,Params.save,SimParams.fname,Params.show,Params.dtime);
end
...
if Plots.waterfallNoise == 'y'
    waterfallNoise(Name,SimData.PC_noise,SimData.IO_spikes,Params.timepreceding,Params.save,SimParams.fname,Params.show,Params.dtime);
end
...
function plotCStrigNoise(Name,IO_spikes,PC_noise,IO_Vs,Noise_t,timepreceding,save,fname,show,dtime)
if iscell(PC_noise)
    [~,colIO] = size(PC_noise);
else
    [colIO,~] = size(PC_noise);
...
%         timepreceding = 300/dtime;%spikes{i,1}(spiketimes{i,1}(1))/(dtime/1000)-2;
    ...
    [~,colPC] = size(PC);
    for ss = 1:colsptim
        start = int64(spikes{i,1}(spiketimes{i,1}(ss))/(dtime/1000)-timepreceding{1,1});
        finish = int64(spikes{i,1}(spiketimes{i,1}(ss))/(dtime/1000)+timepreceding{1,1});
        if start < 0
           continue
        end
        if finish > colPC
        ...
        counter = counter + 1;
        spike_time{i,1}(end+1) = spikes{i,1}(spiketimes{i,1}(ss))/(dtime/1000); 
        ...
        xaxis = xaxis(2:end).*dtime;
        subplot(2,1,1);hold on;
        plot(xaxis,PC(start:finish)/10^-9);hold on;
        xlabel('t (msecond)')
        ylabel('I (nA)')
        title('Noise Cell '+ string(i)+' '+Name)
        xlim([-timepreceding{1,1}*dtime  timepreceding{1,1}*dtime])
...
if save == "True"
    for i=1:colIO 
        saveas(h(i), fullfile(fname,sprintf('CS_Noise_%d',i)), 'jpeg');
    end
end
...
for i=1:colIO 
    ...
    for j = range 
        aa = aa+1;
        [~,col1]=size(PC_noises{i,1}{j,1});
        if col1 > col
            PC_noises{i,1}{j,1} = PC_noises{i,1}{j,1}(1:col);
        end
    ...
    for j = range 
        aa = aa+1;
        [~,col2]=size(IOs{i,1}{j,1});
        if col2 > col
            IOs{i,1}{j,1} = IOs{i,1}{j,1}(1:col);
        end
    ...
    for ss = 1:colsptim-2
        spike_time{i,1}(end+1) = spikes{i,1}(spiketimes{i,1}(ss))/(dtime/1000); 
        start = int64(spike_time{i,1}(ss)-timepreceding{1,1});
        finish = int64(spike_time{i,1}(ss)+timepreceding{1,1});
        if start < 0
            continue
        end
        if start > 0
    ...
    [~,colxax] = size(Noise_t(start:finish));
    xaxis = -colxax/2:colxax/2;
    xaxis = xaxis(1:end-1).*dtime;
%     subplot(colIO,1,(i));hold on;
    yyaxis left
...
%     subplot(colIO,1,(i));hold on;
    ...
    if save == "True"
        saveas(h(:), fullfile(fname,char('CS_Triggered_Noise_Averages_'+string(i))), 'jpeg');
    end
end
end

function plotCStrigSS(Name,IO_spikes,PC_v,IO_Vs,Noise_t,PC_spikes,PC_firingrate,timepreceding,save,fname,show,dtime)
if iscell(IO_spikes)
...
%     timepreceding{i,1} = spikes{i,1}(spiketimes{i,1}(1))/(dtime/1000);
    [~,colsptim] = size(spiketimes{i,1});
    xaxis = cell(colsptim-2,1);
...
if save == "True"
    for i=1:colIO 
        saveas(h(i), fullfile(fname,sprintf('IO_PC_%d',i)), 'jpeg');
    end
end
...
for i=1:colIO 
    ...
    for j = range 
        aa = aa+1;
        [~,col22]=size(PC_vs{i,1}{j,1});
        if col22 > col
            PC_vs{i,1}{j,1} = PC_vs{i,1}{j,1}(1:col);
        end
...
%     subplot(colIO,1,(i));hold on;
    ...
    if save == "True"
      saveas(h, fullfile(fname,char('CS_Triggered_PC_Spike_Averages_'+string(i))), 'jpeg'); 
    end
end
...
for i=1:colIO 
    ...
    for j = range 
        aa = aa+1;
        [~,col12]=size(PC_vs1{i,1}{j,1});
        if col12 > col
            PC_vs1{i,1}{j,1} = PC_vs1{i,1}{j,1}(1:col);
        end
...
%     subplot(colIO,1,(i));hold on;
    ...
    if save == "True"
      saveas(h, fullfile(fname,char('CS_Triggered_PC_Firing_Averages_'+string(i))), 'jpeg'); 
    end
end
end

function plotCStrigDCN(Name,IO_spikes,IO_Vs,Noise_t,DCN_spikes,DCN_v,timepreceding,save,fname,show,dtime)
if iscell(IO_spikes)
...
for i=1:colIO 
%         figure(); hold on;
%         timepreceding{1,1} = spikes{i,1}(spiketimes{i,1}(1))/(dtime/1000)-2;
    [~,colsptim] = size(spiketimes{i,1});
    xaxis = cell(colsptim-2,1);
...
for i=1:colIO 
    ...
    for j = range 
        aa = aa+1;
        [~,col23]=size(DCNs{i,1}{j,1});
        if col23 > col
            DCNs{i,1}{j,1} = DCNs{i,1}{j,1}(1:col);
        end
...
%     subplot(colIO,1,(i));hold on;
    ...
    legend('DCN','IO_{Vs}');
    if save == "True"
      saveas(h, fullfile(fname,char('CS_Triggered_DCN_Spike_Averages_'+string(i))), 'jpeg'); 
    end
end
end

function plotDCNtrigNoise(Name,DCN_spikes,PC_noise,DCN_v,Noise_t,timepreceding,save,fname,show,dtime)
if iscell(PC_noise)
    [~,colDCN] = size(PC_noise);
else
    [colDCN,~] = size(PC_noise);
end
...
for i=1:colDCN 
    ...
    for jj = range 
        aa = aa+1;
        [~,col1]=size(PC_noises{i,1}{jj,1});
        if col1 > col
            PC_noises{i,1}{jj,1} = PC_noises{i,1}{jj,1}(1:col);
        end
        sumPCnoises(aa,:) = PC_noises{i,1}{jj,1};
    end
    sumPCnoise{i,1} = mean(sumPCnoises);
...
%     subplot(colDCN,1,(i));hold on;
    ...
    if save == "True"
        saveas(h, fullfile(fname,char('DCN_Triggered_Noise_Averages_'+string(i))), 'jpeg');
    end
end
end

function firingplots(Name,PC_firingrate,DCN_firingrate,IO_firingrate,Noise_t,window,save,fname,show,dtime)
start = int64(window(1)/dtime);
finish = int64(window(2)/dtime);
noisee = 0:0.025:120000;
[~,collDCN] = size(DCN_firingrate);
[~,collPC] = size(PC_firingrate);
[~,collIO] = size(IO_firingrate);

h = figure('Renderer', 'painters', 'visible', show);
subplot(2,1,1)
a1 = plot(noisee(1:collDCN)/1000,DCN_firingrate); hold on;
M1 = 'DCN'; hold on;
a2 = plot(noisee(1:collPC)/1000,PC_firingrate); hold on;
M2 = 'PC'; hold on;
legend(M1, M2);
xlabel('t (seconds)')
ylabel('Rate (Hz)')
...
h = figure('Renderer', 'painters', 'visible', show);
subplot(2,1,1)
a1 = plot(noisee(1:collIO)/1000,IO_firingrate); hold on;
M1 = 'IO'; hold on;
a2 = plot(noisee(1:collPC)/1000,PC_firingrate); hold on;
M2 = 'PC'; hold on;
legend(M1, M2);
...
a1 = plot(noisee(1:collIO)/1000,IO_firingrate); hold on;
M1 = 'IO'; hold on;
a2 = plot(noisee(1:collDCN)/1000,DCN_firingrate); hold on;
M2 = 'DCN'; hold on;
legend(M1, M2);
xlabel('t (seconds)')
ylabel('Rate (Hz)')
...
function corrfiringplots(Name,PC_firingrate,DCN_firingrate,IO_firingrate,save,fname,show,dtime)

[r,lags] = xcorr(DCN_firingrate,PC_firingrate,'unbiased');
h = figure('Renderer', 'painters', 'visible', show);
subplot(2,1,1)
plot(lags*dtime/1000,r)
...
function rasterplotPCIO(Name,IO_spikes,PC_spikes,IO_Vs,timepreceding,save,fname,show,dtime)
if iscell(IO_spikes)
    [~,colIO] = size(IO_spikes);
else
    [colIO,~] = size(IO_spikes);
...
for i=1:colIO 
    ...
    for k = 1:size(spiketimes{i,1},2)
        start = spikes{i,1}(spiketimes{i,1}(k))*1000/dtime;
        if k > (size(spiketimes{i,1},2)-1)
            finish = spikes{i,1}(end)*1000/dtime;
        else
            finish = spikes{i,1}(spiketimes{i,1}(k+1))*1000/dtime;
        end
...
for j = 1:length(PC_spikes)
    ...
    for i = 1:row
        ...
        time = timepreceding{1,1};
        if iscell(PC_spikes)
            PCSpikes = PC_spikes{1,j}*1000/dtime;
        else
            PCSpikes = PC_spikes(j,:)*1000/dtime;
        end
        
        start = find(PCSpikes<(trr(i,2)-time), 1, 'last');
        finish = find(PCSpikes<(trr(i,2)+time), 1, 'last');
        
        xx = trr(i,:);
        middleElement = xx(ceil(numel(xx)/2));
        x = xx-middleElement;
        x = x*dtime;
        if x(1) < -timepreceding{1,1}*dtime
            x(1) = 0;
        end
...
if save == "True"
    for i=1:length(PC_spikes) 
        saveas(h(i), fullfile(fname,sprintf('RasterPlot_%d',i)), 'jpeg');
    end
end
end

function waterfallIOVs(Name,IO_spikes,IO_Vs,timepreceding,save,fname,show,dtime)
if iscell(IO_spikes)
    [~,colIO] = size(IO_spikes);
else
    [colIO,~] = size(IO_spikes);
...
if save == "True"
    for i=1:colIO
        saveas(h(i), fullfile(fname,sprintf('Waterfall_IO_%d',i)), 'jpeg');
    end
end
end

function waterfallNoise(Name,PC_noise,IO_spikes,timepreceding,save,fname,show,dtime)
if iscell(PC_noise)
    [~,colIO] = size(PC_noise);
else
    [colIO,~] = size(PC_noise);
...
if save == "True"
    for i=1:colIO
        saveas(h(i), fullfile(fname,sprintf('Waterfall_Noise_%d',i)), 'jpeg');
    end
end
end

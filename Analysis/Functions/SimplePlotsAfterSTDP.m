function SimplePlotsAfterSTDP(Type,Simulation,Data,Params)
if Type == "Coupled"
...
%% PC Plots
if Plots.PC == 'y';
    PCplot(Name,SimData.PC_spikes,SimData.Noise_t,SimData.PC_spikings,Params.window,Params.save,SimParams.fname,Params.show);
end
...
function plotNoise(Name,Noise_I,Noise_t,save,fname,show)
    ...
    for i = 1:rownoise
        a(i) = plot(Noise_t/1000,Noise_I(i,:)/10^-9); 
        M(i) = 'Noise '+string(i);
        hold on;
    end
...
%     ylabel('I (nA)')
%     if save == "True"
%         saveas(h, fullfile(fname,'Noises_PC_Noise'), 'jpeg');
%     end
%     h = figure('Renderer', 'painters', 'visible', show); hold on;
%     for ii = 1:rowPCnoise
%         subplot(2,1,(ii));
%         aPC(ii) = plot(Noise_t/1000,PC_noise(ii,:)/10^-9,colors(ii)); MPC(ii) = 'Noise PC'+string(ii); hold on;
%         for i = 1:rownoise
%             a(i) = plot(Noise_t/1000,Noise_I(i,:)/10^-9); M(i) = 'Noise '+string(i); hold on;
%         end
...
noisee = 0:0.025:240000;
[~,colnoise]=size(noisee);
for i = 1:colIO
    h = figure('Renderer', 'painters', 'visible', show); hold on;
    [~,collPC] = size(PC_spikings{i,1});
    [~,collDCN] = size(DCN_spikings{i,1});
    a(1) = plot(noisee(1:collPC)/1000,PC_spikings{i,1}*1000); M(1) = 'PC '+string(i);hold on;
    a(2) = plot(noisee(1:collDCN)/1000,DCN_spikings{i,1}*1000);M(2) = 'DCN '+string(i);hold on;
    a(3) = plot(Noise_t/1000,IO_Vs(i,:)*1000); M(3) = 'IO '+string(i);
...
for i = 1:colIO
    h = figure('Renderer', 'painters', 'visible', show); hold on;
    a(1) = plot(Noise_t(start:finish)/1000,PC_spikings{i,1}(start:finish)*1000); M(1) = 'PC '+string(i);hold on;
    a(2) = plot(Noise_t(start:finish)/1000,DCN_spikings{i,1}(start:finish)*1000);M(2) = 'DCN '+string(i);hold on;
    a(3) = plot(Noise_t(start:finish)/1000,IO_Vs(i,(start:finish))*1000,'g'); M(3) = 'IO '+string(i);
...
function PCplot(Name,PC_spikes,Noise_t,PC_spikings,window,save,fname,show)
if iscell(PC_spikes)
    [~,colIO] = size(PC_spikes);
else
    [colIO,~] = size(PC_spikes);
...
% for i = 1:colIO
%     subplot(2,1,i)
%     a(i) = plot(Noise_t(start:finish)/1000,PC_spikings{i,1}(start:finish)*1000,colors(i)); M(i) = 'PC '+string(i);hold on;
%     legend([a(i)], [M(i)]);
%     xlabel('t (seconds)')
%     ylabel('V (mV)')
...
% end
% if save == "True"
%     saveas(h, fullfile(fname,'PC_Responses_Both_Window'), 'jpeg');
% end
end
...
% for i = 1:colIO
%     subplot(2,1,i)
%     a(i) = plot(Noise_t/1000,DCN_spikings{i,1}*1000,colors(i)); M(i) = 'DCN '+string(i);hold on;
...
% end
% if save == "True"
%     saveas(h, fullfile(fname,'DCN_Responses_Both'), 'jpeg');
% end
...
% xlim([window(1)/1000+Noise_t(1)/1000 window(2)/1000+Noise_t(1)/1000])
if save == "True"
    saveas(h, fullfile(fname,'DCN_Responses_Window'), 'jpeg');
end
end
...
% for i = 1:colIO
%     subplot(2,1,i)
%     a(i) = plot(Noise_t(start:finish)/1000,DCN_spikings{i,1}(start:finish)*1000,colors(i)); M(i) = 'DCN '+string(i);hold on;
...
% end
% if save == "True"
%     saveas(h, fullfile(fname,'DCN_Responses_Both_Window'), 'jpeg');
% end
end
...
for i = 1:colIO
    h = figure('Renderer', 'painters', 'visible', show); hold on;
    a(i) = plot(Noise_t(start:finish)/1000,IO_Vs(i,(start:finish))*1000); M(i) = 'IO '+string(i);
...
title('IO Response '+string(i)+' '+Name+' Window')
legend([a(i)], [M(i)]);
xlabel('t (seconds)')
ylabel('V (mV)')
if save == "True"

from G_NeuronGroups_Plasticity import *
...
A_IO = -0.1/float(exp_runtime/msecond)#Apre*taupre/taupost*1.05
...
mon_N_PC_Coupled = StateMonitor(conn_N_PC_Coupled , ['a_PC','a_IO','noise_source','PC_target','weight','I','delta_weight','new_weight'], record=True, dt=dt_rec)
# Set up the labels
...
conn_N_PC_Coupled.weight = 'abs(1-(abs(((conn_target-noise_source)/N_Cells_PC))))'
...
# LTD from IO cells:
S_IO_N_Coupled = Synapses(IO_Coupled_STDP, conn_N_PC_Coupled, on_pre='a_IO_post += A_IO*(new_weight_post*I_post)/nA', method='euler',name = 'dummy_IO_Coupled',dt=dt)  # where f is some function
...
mon_N_PC_Uncoupled = StateMonitor(conn_N_PC_Uncoupled , ['a_PC','a_IO','noise_source','PC_target','weight','I','delta_weight','new_weight'], record=True,dt=dt_rec)
# Set up the labels

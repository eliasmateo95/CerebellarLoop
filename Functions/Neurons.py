...
def Noise_run(Parameters):
    ...
    PF_OU_Values["sigma"] = [Parameters["sigma_Noise"]*nA]*Parameters["N_Cells_PF_OU"]
...
#     if Parameters["N_Cells_PF_events"] == 2:
#         if Parameters["exp_run"] > 10*second: transient = 10*second
#         else: transient = 1*second
#         # no pulse transient
#         t = transient
#         PF.pulse = PF_Values["no_pulse"]*nA 
#         run(transient)
#         while t < Parameters["exp_run"]:
#             PF.pulse = PF_Values["CS_pulse"]*nA
#             if t+(PF_Values["CS_pulse_duration"]) > (Parameters["exp_run"]): 
#                 # stop simulation if cons stim leads to out of exp_run run time
#                 run((Parameters["exp_run"])-t)
#                 break
#             # now run with pulse for CS duration
#             run(PF_Values["CS_pulse_duration"])
#             t += (PF_Values["CS_pulse_duration"])
#             PF.pulse = PF_Values["US_pulse"]*nA
#             if t+(PF_Values["US_pulse_duration"]) > (Parameters["exp_run"]): 
#                 # stop simulation if uc stim leads to out of exp_run run time
#                 run((Parameters["exp_run"])-t)
#                 break
#             # now run with pulse for US duration
#             run(PF_Values["US_pulse_duration"])
#             t += (PF_Values["US_pulse_duration"])
#             PF.pulse = PF_Values["no_pulse"]*nA 
#             if t+(PF_Values["interpulse_duration"]) > (Parameters["exp_run"]): 
#                 run((Parameters["exp_run"])-t)
#                 break
#             else: 
#                 run(PF_Values["interpulse_duration"])
#                 t += (PF_Values["interpulse_duration"])
#     elif Parameters["N_Cells_PF_events"] == 1:
#         if Parameters["exp_run"] > 10*second: transient = 10*second
#         else: transient = 1*second
#         # no pulse transient
#         t = transient
...
def Noise_neuron(Noise_record,N_Noise,I_recorded,Noise_Gain,exp_run,dt,dt_rec):
    ...
    return Noise, StateMonitor(Noise, variables=['I','weight','noise_gain'], record=Noise_record, dt=dt_rec)
...
#     PC_Statemon = StateMonitor(PC, variables = ['v','I_Noise'], record=PC_record, dt=dt_rec)
    PC_Statemon = StateMonitor(PC, variables = ['v','w','I_Noise','recent_rate','recent_rate_100'], record=PC_record, dt=dt_rec)
...
def IO_neurons(IO_record,N_Cells_IO,Noise_frozen,IO_Values,thresh,I_recorded_copy,shunting,f0,dt,dt_rec):
    ...
    for IO_num in range(0, N_Cells_IO, 1):
        ...
        record_arr = ['Vs','I_IO_DCN','g_Ca_l','I_OU_Copy','I_OU','I_c']
        if shunting == True:
            record_arr = ['Vs','I_IO_DCN','g_Ca_l','I_OU_Copy','I_OU','g_c','I_c']
...
def IO_equations(Noise_frozen,I_recorded_copy,shunting,f0,dt):
    ...
    if shunting == True:
        eqs_I_C = ''' 
            dg_c/dt = (0.0015*mS/cm**2-g_c)/(100*ms)                                            : siemens*meter**-2
            '''
        eqs_IO_V+=eqs_I_C
    if f0 == 'eye_blink':
        print('eye_blink IO_OU_Copy')
        eqs_IO_V_copy = '''
...
# #         '''
    ...
    if f0 == 'eye_blink': 
        ...
        print('eye_blink IO_OU_Copy added')

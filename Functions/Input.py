...
#         yf = (yf_no_Plasticity)
#         yf_time = Parameters["I_0_Noise"]+np.fft.irfft(yf)
#         energ_x = sum(np.abs(x)**2)
#         energ_yf = sum(np.abs(yf_time)**2)
#         new_signal = yf_time*(energ_x**0.5)/(energ_yf**0.5)
#         filtered_noise.append(new_signal*nA)
...
def sims(Parameters,simulation,Noise_I):
    ...
    if simulation == 'light': 
        print(simulation)
        Parameters["N_Cells_PF_events"] = 1
        PF_Values = {}
        PF_Values["CS_pulse"] = [0]*(Parameters["N_Cells_PF_OU"]-Parameters["N_Cells_PF_events"])
        if Parameters["N_Cells_PF_events"] == 2: PF_Values["US_pulse"] = [0]*(Parameters["N_Cells_PF_OU"]-Parameters["N_Cells_PF_events"])
        if Parameters["N_Cells_PF_events"] >= 1:
            ...
            PF_Values["CS_pulse"].extend(PF_Values["CS_pulse_amplitude"])
        if Parameters["N_Cells_PF_events"] == 2:
            PF_Values["US_pulse"].extend(PF_Values["CS_pulse_amplitude"])
            PF_Values["US_pulse_duration"] = Parameters["US_pulse_duration"]
            PF_Values["US_pulse_amplitude"] = [Parameters["US_pulse_amplitude"]]
            PF_Values["CS_pulse"].extend([0])
            PF_Values["US_pulse"].extend(PF_Values["US_pulse_amplitude"])   
            PF_Values['US_pulse'] = np.reshape(PF_Values['US_pulse'],[5,1]) 
        PF_Values['CS_pulse'] = np.reshape(PF_Values['CS_pulse'],[5,1])    
        ...
        if Parameters["N_Cells_PF_events"] >= 1:
            ...
            while step <= int(Parameters["exp_run"]/Parameters['dt']):
        ...
        if Parameters["N_Cells_PF_events"] == 2:
            PF_Values["US_pulse"].extend(PF_Values["CS_pulse_amplitude"])
            PF_Values['US_pulse_initial'] = [0,0,0,0,Parameters["US_pulse_amplitude"]]
            PF_Values["US_pulse_duration"] = Parameters["US_pulse_duration"]
            PF_Values["US_pulse_amplitude"] = [Parameters["US_pulse_amplitude"]]
            PF_Values["CS_pulse"].extend([0])
            PF_Values["US_pulse"].extend(PF_Values["US_pulse_amplitude"])   
            PF_Values['US_pulse'] = np.reshape(PF_Values['US_pulse'],[5,1]) 
            PF_Values['US_pulse_initial'] = np.reshape(PF_Values['US_pulse_initial'],[5,1]) 
        PF_Values['CS_pulse'] = np.reshape(PF_Values['CS_pulse'],[5,1])    
        I_pulse = zeros((Parameters["N_Cells_PF_OU"],int(Parameters["exp_run"]/Parameters['dt'])))
        transient = int(10*second/Parameters['dt'])
        iti_arr = [10]
...
# def sims(Parameters,simulation,Noise_I):
#     Parameters["N_Cells_PF_OU"]  = 5
#     if simulation == 'light': Parameters["N_Cells_PF_events"] = 1
#     elif simulation == 'eye_blink': Parameters["N_Cells_PF_events"] = 2
#     PF_Values = {}
#     PF_Values["CS_pulse"] = [0]*(Parameters["N_Cells_PF_OU"]-Parameters["N_Cells_PF_events"])
#     if Parameters["N_Cells_PF_events"] == 2: PF_Values["US_pulse"] = [0]*(Parameters["N_Cells_PF_OU"]-Parameters["N_Cells_PF_events"])
#     if Parameters["N_Cells_PF_events"] >= 1:
...
#                 step = start + int(PF_Values['US_pulse_duration']/Parameters['dt']) 
#                 I_pulse[:,start:step] = PF_Values['US_pulse']            
#             start = step + int(PF_Values['interpulse_duration']/Parameters['dt'])
#             step = start + CS_pulse_dur
#             I_pulse[:,start:step] = PF_Values['CS_pulse']

#     return (Noise_I/nA*amp+I_pulse)*nA, I_pulse[-1], iti_arr
...
# def filtered_noise(Parameters,f0,Noise_I,order):
#     exp_run = Parameters["exp_run"]
#     fs = 1/(Parameters['dt_rec']/second)
#     duration = Parameters["exp_run"]/second
...
#     highcut = lowcut + 5
#     for N_Noise in range(Parameters["N_Cells_PF"]):
#         x = Noise_I[N_Noise]/nA*amp
#         new_x = x[::-1]
#         new_x = np.append(new_x,x)#,axis=1)
#         new_x = np.append(new_x,x[::-1])#,axis=1)
#         y = butter_bandpass_filter(new_x, lowcut, highcut, fs, order)
#         xf = rfftfreq(N, 1 / fs)
...
#         yf = yf[1:]
#         xf = xf[1:]
#         yf_time = np.fft.irfft(yf_no_Plasticity)
#         energ_x = sum(np.abs(x)**2)
#         energ_yf = sum(np.abs(yf_time)**2)
#         new_signal = np.fft.irfft(yf)*(energ_x**0.5)/(energ_yf**0.5)
#         new_signal = new_signal[int(exp_run/msecond):2*int(exp_run/msecond)]
#         filtered_noise.append((1.5+new_signal)*nA)
#     return filtered_noise

# def filtered_noise(Parameters,f0,Noise_I):
#     fs = 1/(Parameters['dt_rec']/second)
#     duration = Parameters["exp_run"]/second
#     # Number of samples in normalized_tone
#     N = int(fs * duration)
...
#         energ_yf = sum(np.abs(yf_time)**2)
#         new_signal = np.fft.irfft(yf)*(energ_x**0.5)/(energ_yf**0.5)
#         filtered_noise.append(new_signal)
#     return filtered_noise
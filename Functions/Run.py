...
#     print(zebrin_ff)
    ...
    if IO_shunting == True:
         eqs_IO_syn = ''' 
          g_c_clip = ((0)*mS/cm**2*int((g_c_pre)<0*mS/cm**2) + g_c_pre*int((g_c_pre)>=0*mS/cm**2))   : siemens*meter**-2
          I_c_pre = g_c_clip*(0.6*e**(-((Vd_pre/mvolt-Vd_post/mvolt)/50)**2) + 0.4)*(Vd_pre-Vd_post) : metre**-2*amp (summed)
          '''
    print(IO_shunting, eqs_IO_syn)
...
#     tau_thresh_M_val = int(seed_number.replace('New_BCM_tanh_new_weight_new_theta_tau_', ''))*ms
    ...
    PC_DCN_val = int(parameters_value[zebrin]['Uncoupled']['PC_DCN'])
    PC_DCN_val_Coupled = int(parameters_value[zebrin]['Coupled']['PC_DCN'])
    leak_val = parameters_value[zebrin]['Uncoupled']['leak']
    leak_val_Coupled = parameters_value[zebrin]['Coupled']['leak']
    sigma_OU_val = parameters_value[zebrin]['Uncoupled']['sigma_OU']
    sigma_OU_val_Coupled = parameters_value[zebrin]['Coupled']['sigma_OU']
    b_OU_val = parameters_value[zebrin]['Uncoupled']['b_OU']
    ...
    ############################# IO ##################################
    ###################################################################
    IO, IO_Statemon, IO_Spikemon, IO_rate = IO_neurons(IO_record,N_Cells_IO,Noise_frozen,IO_Values,IO_thresh,I_recorded_copy,IO_shunting,f0,dt,dt_rec)
    ###################################################################
    ############################# PF ##################################
    ###################################################################
...
#         print(w_IO_DCN_val)
    DCN_IO_Synapse = DCN_IO_syn(DCN,IO,N_Cells_DCN,N_Cells_IO,w_IO_DCN_val,IO_shunting,dt,dt_rec)

...
#     PC_DCN_Synapse_Sources = np.repeat(np.arange(N_Cells_PC), 10)

#     # create an array of targets
#     PC_DCN_Synapse_Targets = np.zeros(N_Cells_PC * 10, dtype=int)

#     for i in range(N_Cells_PC):
#         # choose 10 unique targets for each source
#         available_targets = np.setdiff1d(np.arange(N_Cells_DCN), PC_DCN_Synapse_Targets[:i*10])
#         if available_targets.size == 0:
#             available_targets = np.arange(N_Cells_DCN)
#         PC_DCN_Synapse_Targets[i*10:(i+1)*10] = np.random.choice(available_targets, size=10, replace=False)
...
#         target_indices = np.random.choice(range(N_Cells_PC), 10, replace=True)
